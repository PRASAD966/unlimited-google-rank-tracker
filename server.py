...
import sys
# Eventlet removed to prevent Playwright threading deadlocks
...
import time, os, json, uuid, datetime, threading, argparse, random, socket
...
# Task and semaphore configuration
# 32GB Server Optimization: Max 20 concurrent browsers
try:
    max_tasks = int(os.getenv("MAX_CONCURRENT_TASKS", "20"))
except ValueError:
    max_tasks = 20
...
# Production: use ProxyFix so url_for() generates https:// links (OAuth redirects)
# DO NOT use SERVER_NAME — it breaks routing if request Host header differs
...
# Use standard threading backend for Playwright compatibility
socketio = SocketIO(
    app,
...
def get_task(run_id):
    ...
    try:
        ...
        try:
            row["keywords"] = json.loads(row.get("keywords_json", "[]")) if row.get("keywords_json") else []
        except:
            row["keywords"] = []
            
        row["run_id"] = row.get("id")
        row["user"] = row.get("user_email")
        row["cancelled"] = bool(row.get("cancelled", 0))
        return row
    except Exception as e:
...
def save_task(run_id, task):
    """
    Updates the status, cancelled state, keywords, and SID of a task.
    In the new DB-driven architecture, this modifies the 'runs' table.
    """
    conn = get_db_connection()
    if not conn: return
    try:
        cursor = conn.cursor()
        keywords_json = json.dumps(task.get("keywords", []))
        status = task.get("status", "pending")
        cancelled = 1 if task.get("cancelled") else 0
        sid = task.get("sid")
        
        cursor.execute("""
            UPDATE runs 
            SET status = %s, cancelled = %s, keywords_json = %s, sid = %s
            WHERE id = %s
        """, (status, cancelled, keywords_json, sid, run_id))
        conn.commit()
    except Exception as e:
...
def delete_task_from_disk(run_id):
    """
    Deprecated: No longer deletes from disk. Instead, marks task as finished or stopped.
    """
    # Simply set status to 'stopped' so it doesn't show up as 'active' anymore.
    conn = get_db_connection()
    if not conn: return
    try:
        cursor = conn.cursor()
        cursor.execute("UPDATE runs SET status = 'cancelled', cancelled = 1 WHERE id = %s", (run_id,))
        conn.commit()
    except Exception:
        pass
    finally:
...
def get_active_tasks_for_user(user_email):
    ...
    try:
        ...
        cursor.execute("""
            SELECT * FROM runs 
            WHERE user_email = %s 
            AND status IN ('started', 'waiting_for_socket', 'ready_to_start')
            AND cancelled = 0
        """, (user_email,))
        ...
        print(f"Error in get_active_tasks: {e}")
    finally:
        if conn: conn.close()
    return active

def set_task_for_user(user, task_obj):
    # This was used to map user to current task, 
    # but we now use run_id for everything. Deprecated but kept for safety.
    pass
...
def find_keyword_rank(page, keyword, target_domain, max_pages=10, location=None, country_code=None, start_page=0):
    ...
    for p in range(start_page, max_pages):
        ...
        try:
            ...
            page.goto(url, referer="https://www.google.com/", wait_until="domcontentloaded", timeout=60000)
            
            if page.url == "about:blank":
                raise Exception("Proxy socket died immediately (about:blank).")
            
            # Give JS time to render after domcontentloaded (randomized human-like delay)
            time.sleep(random.uniform(1.2, 3.5))

            # Wait for organic results OR detect a block/error page
            results_appeared = False
            try:
                page.wait_for_selector("#search .g, #rso .g, #rso .MjjYud, #rso .xpd", timeout=12000)  # relaxed timeout for proxies
            ...
            try:
                for btn_text in ["Accept all", "I agree", "Accept everything", "Agree"]:
                    ...
                    if btn.is_visible(timeout=500):  # 1s→500ms — consent popup is instant
                        ...
                        try:
                            page.wait_for_selector("#search .g, #rso .g, #rso .MjjYud, #rso .xpd", timeout=15000)
                            results_appeared = True
                        except:
                            pass
                        break
            ...
            # 3. Inject Human Behavior 
            # Subtly moving the mouse and scrolling bypasses passive heuristic monitoring telemetry.
            try:
                # Random mouse sweep
                ...
                # Random scroll to "read" results
                scroll_amount = random.randint(400, 1800)
                page.mouse.wheel(0, scroll_amount)
                time.sleep(random.uniform(0.8, 2.5))
                
                # Final randomized jiggle 
                page.mouse.move(x + random.randint(-40, 40), y + random.randint(-40, 40), steps=random.randint(3, 8))
                time.sleep(random.uniform(0.2, 0.7))
            except Exception as e_human:
                print(f"DEBUG: Human behavior simulation failed softly: {e_human}")
            ...
            () => {
                ...
                // Identify ad regions to exclude
                ...
                function isInsideAd(node) {
                    if (adNodes.some(ad => ad.contains(node))) return true;
                    // Strict "Sponsored" check: look for exact text match in small spans
                    const p = node.closest('.g, .MjjYud, div[data-text-ad], div[jscontroller], .xpd');
                    if (p) {
                        const spans = Array.from(p.querySelectorAll('span, div'));
                        if (spans.some(el => {
                            const t = el.textContent.trim().toLowerCase();
                            return t === 'sponsored' || t === 'ad' || t.startsWith('sponsored •');
                        })) return true;
                    }
                    return false;
                }

                // Gather ALL potential block containers where a Google organic result might live
                // #rhs is the right Knowledge Panel. The rest are main feed items.
                const blocks = document.querySelectorAll('.g, .MjjYud, .xpd.open-exact-match, .ydpP0d, .Ww4Afe, .hlcw0c, [data-sokoban-container], #rhs');

                blocks.forEach(block => {
                    if (block.dataset.counted) return; // Already processed this block via a parent
                    
                    if (isInsideAd(block)) return;

                    // Skip "People also ask", "Videos", "Related searches" and generic carousels
                    const blockText = block.innerText || "";
                    const lowerText = blockText.substring(0, 150).toLowerCase();
                    if (lowerText.includes('people also ask') || 
                        lowerText.includes('images for') ||
                        lowerText.includes('top stories') ||
                        lowerText.includes('related searches') ||
                        lowerText.includes('discover more places') ||
                        lowerText.includes('more to ask') ||
                        block.querySelector('.Wt5Tfe') || block.querySelector('[data-initq]')) {
                        return;
                    }

                    // Find the PRIMARY destination link of this block.
                    // We ONLY want traditional organic links that have an <h3> tag or are in .yuRUbf
                    let primaryLink = null;
                    const traditionalLink = block.querySelector('a:has(h3), h3 a, h2 a, [role="heading"] a, .yuRUbf > a, .yuRUbf > div > a, .yuRUbf > div > span > a');
                    
                    if (traditionalLink) {
                        primaryLink = traditionalLink;
                    }

                    if (primaryLink) {
                        const href = primaryLink.href;
                        // Avoid google properties except specific dynamic outs
                        if (!href.includes('google.') || href.includes('/url?')) {
                            // Extract actual target from google /url?q= wrapper if present
                            let finalUrl = href;
                            if (href.includes('/url?q=')) {
                                try {
                                    finalUrl = new URL(href).searchParams.get('q') || href;
                                } catch(e){}
                            }
                            
                            // Prevent tracking anchors
                            finalUrl = finalUrl.split('#')[0];

                            if (!seen.has(finalUrl)) {
                                seen.add(finalUrl);
                                organic.push(finalUrl);
                            }
                            
                            // Mark this block and ALL sub-blocks (sitelinks) as counted to prevent sitelink inflation!
                            block.dataset.counted = "true";
                            const subBlocks = block.querySelectorAll('.g, .MjjYud, .xpd, .ydpP0d, .Ww4Afe, .hlcw0c, [data-sokoban-container]');
                            subBlocks.forEach(sub => sub.dataset.counted = "true");
                        }
                    }
                });
            ...
            if "timeout" in err_msg.lower() or "closed" in err_msg.lower() or "crashed" in err_msg.lower() or "tunnel_connection_failed" in err_msg.lower():
...
def launch_worker_browser(extension_path=None):
    ...
    args.extend([
        ...
        "--disable-extensions",
        "--disable-features=AudioServiceOutOfProcess,IsolateOrigins,site-per-process,Translate,MediaRouter,DialMediaRouteProvider",
        ...
        "--blink-settings=imagesEnabled=false",  # Hardware-level image block (backup layer)
        "--disable-remote-fonts",                # Block remote web font downloads
        "--disable-web-resources",               # Reduce unnecessary web resource loads
        "--dns-prefetch-disable",                # Disable speculative DNS lookups (saves bandwidth)
        "--no-proxy-server-bypass",              # Ensure all traffic routes through proxy
    ])
...
def create_browser_context(browser, geolocation=None, timezone_id="UTC", locale="en-US", target_country=None):
    ...
    # Dynamically build UA to match exact Chromium version but without 'HeadlessChrome'
    import re
    base_version = re.split(r'[- ]', browser.version)[0]
    user_agent = f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{base_version} Safari/537.36"
    ...
    if static_proxy:
        ...
        if PROXY_USERNAME and PROXY_PASSWORD:
            # SmartProxy Session Rotation: Append country and session ID to force a new local IP
            # SmartProxy format: username_country-us_session-random123
            if "smartproxy" in PROXY_SERVER.lower() and "_session-" not in PROXY_USERNAME:
                session_id = str(uuid.uuid4().hex[:8])
                base_username = PROXY_USERNAME
                if target_country and "_country-" not in base_username:
                    rotated_username = f"{base_username}_country-{target_country.lower()}_session-{session_id}"
                else:
                    rotated_username = f"{base_username}_session-{session_id}"
                proxy_config["username"] = rotated_username
            else:
    ...
    try:
        # We rely on Brave's native fingerprinting protections instead of injecting raw JS overrides
    ...
    try:
        ...
        def worker_loop(worker_idx):
            ...
            def init_context():
                ...
                # PRE-FLIGHT STEALTH WARMUP: Set SOCS cookie so Google doesn't see a zero-cookie scraper
                try:
                    stealth_script = """
                    ...
                    """
                    page_worker.add_init_script(stealth_script)
                    
                    page_worker.goto("https://www.google.com/", wait_until="domcontentloaded", timeout=20000)
                    time.sleep(1.5)
                    accept_btn = page_worker.locator("button:has-text('Accept all'), button:has-text('I agree')")
                    if accept_btn.count() > 0:
                        accept_btn.first.click(timeout=3000)
                        time.sleep(2) # Allow async /consent API to set SOCS cookies
                except Exception as e_warmup:
                    print(f"DEBUG: [Worker {worker_idx}] Pre-flight warmup skipped/timeout: {str(e_warmup)[:100]}")
                ...
                threading.Thread(target=_log_proxy_ip, args=(worker_idx,), daemon=True).start()
                
                # BANDWIDTH ROUTING: Block all non-essential resources to save proxy cost and speed up extraction
                try:
                    def should_block(request):
                        resource_type = request.resource_type
                        url = request.url.lower()

                        # Block heavy/visual resource types outright
                        if resource_type in [
                            "image", "media", "manifest",
                            "stylesheet", "font",  # CSS & web fonts not needed for rank extraction
                            "ping",                # beacon/ping requests waste proxy bandwidth
                            "other",               # catch-all for misc background fetch
                        ]:
                            return True

                        # Block ad networks, trackers, analytics, maps & CDN noise
                        ad_block_patterns = [
                            # Analytics / tag managers
                            "google-analytics.com", "googletagmanager.com",
                            "analytics.google.com", "gtag",
                            # Ads
                            "googleadservices.com", "googlesyndication.com",
                            "doubleclick.net", "adservice.google",
                            "adsystem.google", "pagead2.googlesyndication",
                            # Maps (heavy tiles + API calls)
                            "maps.googleapis.com", "maps.gstatic.com",
                            "maps.google.com", "mapsplatform.google",
                            # Fonts (external stylesheet + woff downloads)
                            "fonts.googleapis.com", "fonts.gstatic.com",
                            # Social / tracking pixels
                            "facebook.com", "facebook.net",
                            "connect.facebook", "staticxx.facebook",
                            "platform.twitter.com", "analytics.twitter.com",
                            # Video / media CDN noise
                            "ytimg.com", "youtube.com/embed",
                            # Misc third-party noise
                            "gravatar.com", "disqus.com",
                            "hotjar.com", "clarity.ms",
                            "intercom.io", "crisp.chat",
                            "cdn.segment.com", "segment.io",
                            "mixpanel.com", "amplitude.com",
                            "bing.com/fd",  # Bing tracking beacons
                        ]
            ...
            try:
                ...
                while True:
                    ...
                    # Desynchronize workers intensely to simulate organic human typing and review rhythms
                    ...
                    try:
                        ...
                        while True:
                            try:
                                ...
                                ip_captcha_count += 1
                                if ip_captcha_count > 0:  # instantly rotate ip
                                    print(f"[Worker {worker_idx}] CAPTCHA/Block detected. Changing IP to fresh SmartProxy Session...")
                                    init_context()
                                else:
                                    # Fallback
                                    pass
                                ...
                                keyword_retry_count += 1
                                e_msg = str(e_search).split("Browser logs:")[0].strip()
                                if keyword_retry_count >= 50:
                                    print(f"[Worker {worker_idx}] Max retries reached for '{k}': {e_msg[:200]}")
                                    break
                                print(f"[Worker {worker_idx}] Search error/Timeout for '{k}' on page {current_start_page}: {e_msg[:150]}... Changing IP and retrying...")
                                try:
                                    init_context()
                                except Exception:
                                    print(f"[Worker {worker_idx}] Browser crashed during standard retry. Restarting fully...")
                                    try:
                                        init_browser()
                                        init_context()
                                    except Exception as e_respawn_inner:
                                        print(f"[Worker {worker_idx}] Warning: Context respawn failed: {e_respawn_inner}")
                                time.sleep(1)
                                chk = get_task(run_id)
                                if chk is None or chk.get("cancelled"):
                                    raise TaskCancelledException("Task was cancelled by user")
                        ...
                        # PROACTIVE ROTATION LOGIC: Rotate IP after exactly 5 keywords to balance speed and safety
                        keywords_processed_in_session += 1
                        if keywords_processed_in_session >= 5:
                        ...
                        if isinstance(e_inner, TaskCancelledException) or "cancelled by user" in str(e_inner):
                            print(f"[Worker {worker_idx}] Task loop broken due to cancellation.")
                            break
                        
                        print(f"[Worker {worker_idx}] Error processing keyword '{k}': {e_inner}")
                        # Prevent rapid-fire failures if browser is broken
                        time.sleep(2)
                        # If browser crashed, we must restart it for the next keyword!
                        print(f"[Worker {worker_idx}] Re-initializing browser environment after outer failure.")
                        try:
                            init_browser()
                            init_context()
                        except Exception as e_respawn:
                            print(f"[Worker {worker_idx}] Warning: Could not cleanly respawn browser: {e_respawn}")
                ...
                try:
                    ...
                    pass
                # Browser cleanup completed in init_context/init_browser
                pass
...
def google_authorize():
    try:
        ...
        # Use full URL since api_base_url is dynamic/unset with server_metadata_url
        resp = google.get('https://www.googleapis.com/oauth2/v3/userinfo')
        user_info = resp.json()
        email = user_info['email']
        
        users = load_users()
        # Direct login: we no longer require a password to be set for Google auth
        if email not in users or users[email] == "google_oauth_user":
            # Ensure profile exists even before password is set (so we have name/pic)
            conn = get_db_connection()
            if conn:
                try:
                    cursor = conn.cursor()
                    ...
                    if not cursor.fetchone():
                        cursor.execute("INSERT INTO user_profiles (email, full_name, profile_image, is_verified, total_credits, used_credits) VALUES (%s, %s, %s, %s, %s, %s)", 
        ...
        return "<h3 style='color:red;text-align:center;'>Google Login Failed. Please try again.</h3>"
...
def start_task_internal(keywords, target_domain, location, project_name, max_pages, user_email, explicit_cost=None):
    ...
    try:
        ...
        if conn:
            ...
            sql = """
                INSERT INTO runs (user_email, project_name, target_domain, location, max_pages, total_keywords, excel_filename, excel_data, timestamp, status, keywords_json)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """
            cursor.execute(sql, (user_email, project_name, target_domain, location, max_pages, len(keywords), None, None, datetime.datetime.now(), 'waiting_for_socket', json.dumps(keywords)))
            conn.commit()
            run_id = cursor.lastrowid
            cursor.close()
            conn.close()
            ...
            # task_obj creation and save_task logic is now integrated into start_task_internal DB record
            return run_id
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # Cleanup zombie tasks from previous run
    try:
        conn = get_db_connection()
        if conn:
            cursor = conn.cursor()
            cursor.execute("""
                ...
                WHERE status IN ('started', 'waiting_for_socket', 'ready_to_start') AND cancelled = 0
            """)
            conn.commit()
            conn.close()
            print("Cleanup: Marked any zombie tasks as failed.")
    except Exception as e:
        print(f"Error cleaning up zombie tasks on startup: {e}")

    # Get local IP
    local_ips = []
    try:
        # Standard way (may return 127.0.0.1 on some setups)
        hostname = socket.gethostname()
        for ip in socket.gethostbyname_ex(hostname)[2]:
            if not ip.startswith("127."):
                local_ips.append(ip)
        
        # Reliable connection-based detection
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        try:
            # Use a public DNS server (doesn't actually send data)
            s.connect(('8.8.8.8', 80))
            ip = s.getsockname()[0]
            if ip not in local_ips and not ip.startswith("127."):
                local_ips.insert(0, ip) # Prefer this one
        except:
            pass
        finally:
            s.close()
    ...
    if not local_ips:
        # Extreme fallback if list still empty but we know we had 192.168.0.128 earlier
        print(f"Network: http://192.168.0.128:{args.port}")
    else:
        for ip in local_ips:
